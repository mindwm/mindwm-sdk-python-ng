...
def render_trigger(params: Params):
    ...
    y['spec']['delivery']['deadLetterSink']['ref'][
        'namespace'] = f"context-{params.context}"
...
def render_kservice(params: Params):
    ...
    y['metadata']['namespace'] = f"context-{params.context}"
    y['spec']['template']['spec']['containers'][0][
        'image'] = f"{params.registry}/{params.name}:{params.version}@{params.digest}"
